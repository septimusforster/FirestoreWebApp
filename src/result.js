...
if(ss && ('masterOfForm' in ss.data || ss.data.isAdmin)){
    ...
    let eotData, thisTerm, percentile, oth;
    ...
    document.forms.namedItem('rez_fom').addEventListener('submit', async (e) => {
        ...
        if (eotReadyStatus) {
            ...
            percentile = fd.get('res') || percentile;
            oth = fd.get('oth') || oth;
            ...
            try {
                ...
                const studentsQuery = ARM == "ENTRANCE" ? query(studentsRef, and(where("admission_year", ">=", MONTH), where("arm", "==", "ENTRANCE"))) : query(studentsRef, where("arm", "==", ARM));
                loaded(40);
                ...
                studentsSnapshot.docs.forEach(d => {
                    if (d.data().admission_no.toUpperCase().includes(DCA) && 'record' in d.data()) {
                        // studentIDs.push({sid: result.id, promo: result.data()?.promo_status || null});
                        studentScores
                    }
                });
                ...
                className = `${configs[7].indexOf(FORM) + 7}th Grade ${ARM}`;
